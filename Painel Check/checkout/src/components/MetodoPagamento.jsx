 ...
 *
 * (Lógica 100% preservada do original. Os imports já estavam corretos para a
 *  pasta components/: ../tracking/* e ./MetodoPagamento.module.css.)
 */
...
export default function MetodoPagamento({ cliente, sessao, onVoltar }) {
  ...
  const formatarValor = (v) =>
    new Intl.NumberFormat(loja?.idioma === 'pt' ? 'pt-BR' : 'es-AR', {
      style: 'currency',
  ...
  return (
    <div className={`${styles.container} fade-up`}>
      ...
      <div className={styles.metodos}>
        <label
          className={`${styles.metodo} ${metodo === 'pix' ? styles.selecionado : ''}`}
        >
          <input
            type="radio"
            name="metodo"
        ...
        <label
          className={`${styles.metodo} ${
            metodo === 'cartao' ? styles.selecionado : ''
          }`}
        >
          <input
            type="radio"
      ...
      <div className={styles.ssl}>🔒 Pagamento 100% seguro e criptografado</div>
    </div>
  );
}
