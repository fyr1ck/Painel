/**
 * checkout-node/src/components/CheckoutPage.jsx
 *
      ...
      <header className={styles.header}>
        ...
        <div className={styles.seguranca}>
          <svg width="12" height="14" viewBox="0 0 12 14" fill="none">
            <path d="M6 1L1 3v4c0 3.18 2.16 6.15 5 6.93C9.84 13.15 11 10.18 11 7V3L6 1z"
      ...
      <div className={styles.steps}>
        {['Dados', 'Endereço', 'Pagamento'].map((nome, i) => (
          <React.Fragment key={nome}>
            <div className={`${styles.step} ${etapa > i ? styles.done : ''} ${etapa === i + 1 ? styles.active : ''}`}>
              <span className={styles.stepNum}>{etapa > i + 1 ? '✓' : i + 1}</span>
              <span className={styles.stepLabel}>{nome}</span>
            </div>
            {i < 2 && <div className={`${styles.stepLine} ${etapa > i + 1 ? styles.done : ''}`} />}
          </React.Fragment>
        ))}
      </div>
